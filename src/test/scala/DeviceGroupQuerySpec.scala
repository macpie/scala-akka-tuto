...
class DeviceGroupQuerySpec extends FunSuite {

  implicit val system = ActorSystem("test-system")

  test("return temperature value for working devices") {
  ...
  test("return TemperatureNotAvailable for devices with no readings") {
    val requester = TestProbe()
    val device1 = TestProbe()
    val device2 = TestProbe()
  ...
  test("return DeviceNotAvailable if device stops before answering") {
    val requester = TestProbe()
    val device1 = TestProbe()
    val device2 = TestProbe()
  ...
  test("return temperature reading even if device stops after answering") {
    val requester = TestProbe()
  ...
  test("return DeviceTimedOut if device does not answer in time") {
    val requester = TestProbe()
    val device1 = TestProbe()
    val device2 = TestProbe()
